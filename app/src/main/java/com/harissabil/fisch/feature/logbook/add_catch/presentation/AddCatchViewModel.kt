...
class AddCatchViewModel @Inject constructor(
    ...
    val state: StateFlow<AddCatchState> = _state.asStateFlow()
    ...
    fun onEvent(event: AddCatchEvent) {
        when (event) {
            AddCatchEvent.SaveIntroShown -> saveIntroShown()
            is AddCatchEvent.SetFishImage -> setFishImage(event.imageBitmaps)
                ...
                addLogbook.data?.let { isUploaded ->
                    ...
                    _eventFlow.emit(
                        UIEvent.ShowSnackbar(
                            if (isUploaded) "Catch data uploaded successfully"
                            else "Failed to upload catch data"
                        )
                    )
                }
            }
    ...
    private fun identifyFish() {
        ...
        viewModelScope.launch((Dispatchers.IO)) {
            try {
                ...
                _state.value = _state.value.copy(fishType = response.text.toString().trim())
                _state.value = _state.value.copy(isIdentifying = false)
            } catch (e: Exception) {
                Timber.tag("IdentifyFish").e(e)
                _state.value = _state.value.copy(isIdentifying = false)
